  ...
  implements CryptoHandler
{
  public byte[] decrypt(CryptoUtils.ICryptoFactory paramICryptoFactory, int paramInt, KeyStore.Entry paramEntry, byte[] paramArrayOfByte)
  {
    ...
    paramInt = localICipher.getBlockSize();
    paramICryptoFactory = new IvParameterSpec(paramArrayOfByte, 0, paramInt);
    localICipher.init(2, ((KeyStore.SecretKeyEntry)paramEntry).getSecretKey(), paramICryptoFactory);
    return localICipher.doFinal(paramArrayOfByte, paramInt, paramArrayOfByte.length - paramInt);
  }
  ...
  public byte[] encrypt(CryptoUtils.ICryptoFactory paramICryptoFactory, int paramInt, KeyStore.Entry paramEntry, byte[] paramArrayOfByte)
  {
    ...
    paramArrayOfByte = localICipher.doFinal(paramArrayOfByte);
    paramEntry = new byte[paramICryptoFactory.length + paramArrayOfByte.length];
    System.arraycopy(paramICryptoFactory, 0, paramEntry, 0, paramICryptoFactory.length);
    System.arraycopy(paramArrayOfByte, 0, paramEntry, paramICryptoFactory.length, paramArrayOfByte.length);
    return paramEntry;
  }
...
/* Location:
 * Qualified Name:     base.com.microsoft.appcenter.utils.crypto.CryptoAesHandler
