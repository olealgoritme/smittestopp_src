  ...
  implements CryptoHandler
{
  ...
  public void generateKey(CryptoUtils.ICryptoFactory paramICryptoFactory, String paramString, Context paramContext)
  {
    ...
    paramICryptoFactory = KeyPairGenerator.getInstance("RSA", "AndroidKeyStore");
    paramICryptoFactory.initialize(new KeyPairGeneratorSpec.Builder(paramContext).setAlias(paramString).setSubject(new X500Principal(a.b("CN=", paramString))).setStartDate(new Date()).setEndDate(localCalendar.getTime()).setSerialNumber(BigInteger.TEN).setKeySize(2048).build());
    paramICryptoFactory.generateKeyPair();
  }
...
/* Location:
 * Qualified Name:     base.com.microsoft.appcenter.utils.crypto.CryptoRsaHandler
