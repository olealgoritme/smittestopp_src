...
public class HttpUtils
{
  ...
  public static boolean isRecoverableError(Throwable paramThrowable)
  {
    ...
    Object localObject = RECOVERABLE_EXCEPTIONS;
    int j = localObject.length;
    for (int i = 0; i < j; i++) {
      if (localObject[i].isAssignableFrom(paramThrowable.getClass())) {
        return true;
      }
    }
    ...
    if (localObject != null)
    {
      ...
      for (i = 0; i < j; i++) {
        if (arrayOfClass[i].isAssignableFrom(localObject.getClass())) {
          return true;
        }
      }
    }
...
/* Location:
 * Qualified Name:     base.com.microsoft.appcenter.http.HttpUtils
