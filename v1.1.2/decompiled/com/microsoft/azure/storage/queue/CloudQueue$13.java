  ...
  extends StorageRequest<CloudQueueClient, CloudQueue, QueuePermissions>
{
  ...
  {
    paramCloudQueueClient = SharedAccessPolicyHandler.getAccessIdentifiers(getConnection().getInputStream(), SharedAccessQueuePolicy.class);
    paramCloudQueue = paramCloudQueueClient.keySet().iterator();
    while (paramCloudQueue.hasNext())
    {
      paramHttpURLConnection = (String)paramCloudQueue.next();
      paramQueuePermissions.getSharedAccessPolicies().put(paramHttpURLConnection, paramCloudQueueClient.get(paramHttpURLConnection));
    }
...
/* Location:
 * Qualified Name:     com.microsoft.azure.storage.queue.CloudQueue.13
