  ...
  extends StorageRequest<CloudQueueClient, CloudQueue, ArrayList<CloudQueueMessage>>
{
  ...
  public ArrayList<CloudQueueMessage> preProcessResponse(CloudQueue paramCloudQueue, CloudQueueClient paramCloudQueueClient, OperationContext paramOperationContext)
  {
    ...
    paramCloudQueueClient = QueueMessageHandler.readMessages(getConnection().getInputStream(), CloudQueue.access$100(paramCloudQueue));
    if (val$options.getEncryptionPolicy() != null)
    {
      paramOperationContext = paramCloudQueueClient.iterator();
      while (paramOperationContext.hasNext())
      {
        paramCloudQueue = (CloudQueueMessage)paramOperationContext.next();
        paramCloudQueue.setMessageContent(val$options.getEncryptionPolicy().decryptMessage(messageContent, val$options.requireEncryption()));
      }
    }
    return paramCloudQueueClient;
  }
...
/* Location:
 * Qualified Name:     base.com.microsoft.azure.storage.queue.CloudQueue.8
