  ...
  extends IdentityProvider<AzureActiveDirectoryOAuth2Strategy, AzureActiveDirectoryOAuth2Configuration>
{
  ...
  public static void initializeCloudMetadata(String paramString, Map<String, String> paramMap)
  {
    ...
    paramString = deserializeClouds(paramMap).iterator();
    while (paramString.hasNext())
    {
      paramMap = (AzureActiveDirectoryCloud)paramString.next();
      paramMap.setIsValidated(true);
      Iterator localIterator = paramMap.getHostAliases().iterator();
      while (localIterator.hasNext())
      {
        String str = (String)localIterator.next();
        sAadClouds.put(str.toLowerCase(Locale.US), paramMap);
      }
    }
    sIsInitialized = true;
  }
  ...
  public static void performCloudDiscovery()
  {
    ...
    {
      HttpCache.flush();
      localObject2 = ((AzureActiveDirectoryInstanceResponse)ObjectMapper.deserializeJsonStringToObject(((HttpResponse)localObject1).getBody(), AzureActiveDirectoryInstanceResponse.class)).getClouds().iterator();
      while (((Iterator)localObject2).hasNext())
      {
        AzureActiveDirectoryCloud localAzureActiveDirectoryCloud = (AzureActiveDirectoryCloud)((Iterator)localObject2).next();
        localAzureActiveDirectoryCloud.setIsValidated(true);
        Iterator localIterator = localAzureActiveDirectoryCloud.getHostAliases().iterator();
        while (localIterator.hasNext())
        {
...
/* Location:
 * Qualified Name:     com.microsoft.identity.common.internal.providers.microsoft.azureactivedirectory.AzureActiveDirectory
