...
public final class FileRequest
{
  ...
  {
    UriQueryBuilder localUriQueryBuilder = BaseRequest.getListUriQueryBuilder(paramListingContext);
    if ((paramEnumSet != null) && (paramEnumSet.size() > 0))
    {
      paramListingContext = new StringBuilder();
      int i = 0;
      if (paramEnumSet.contains(ShareListingDetails.SNAPSHOTS))
      {
        i = 1;
        paramListingContext.append("snapshots");
      }
      if (paramEnumSet.contains(ShareListingDetails.METADATA))
      {
        if (i != 0) {
          paramListingContext.append(",");
        }
        paramListingContext.append("metadata");
      }
      localUriQueryBuilder.add("include", paramListingContext.toString());
    }
    paramURI = BaseRequest.createURLConnection(paramURI, paramFileRequestOptions, localUriQueryBuilder, paramOperationContext);
    paramURI.setRequestMethod("GET");
    return paramURI;
...
/* Location:
 * Qualified Name:     base.com.microsoft.azure.storage.file.FileRequest
