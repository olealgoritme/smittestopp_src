...
public final class PathUtility
{
  ...
  public static URI addToSingleUriQuery(URI paramURI, HashMap<String, String[]> paramHashMap)
  {
    ...
    while (localIterator.hasNext())
    {
      ...
      for (paramHashMap : (String[])localEntry.getValue()) {
        localUriQueryBuilder.add((String)localEntry.getKey(), paramHashMap);
      }
    }
  ...
  public static HashMap<String, String[]> parseQueryString(String paramString)
  {
    ...
    paramString = ((String)localObject).split(paramString);
    for (i = 0; i < paramString.length; i++)
    {
      int j = paramString[i].indexOf("=");
      if ((j >= 0) && (j != paramString[i].length() - 1))
      {
        localObject = paramString[i].substring(0, j);
        String str = paramString[i].substring(j + 1);
        localObject = Utility.safeDecode((String)localObject);
        str = Utility.safeDecode(str);
        String[] arrayOfString = (String[])localHashMap.get(localObject);
        if (arrayOfString == null)
        {
          if (!str.equals("")) {
            localHashMap.put(localObject, new String[] { str });
          }
        }
        else if (!str.equals(""))
        {
...
/* Location:
 * Qualified Name:     com.microsoft.azure.storage.core.PathUtility
