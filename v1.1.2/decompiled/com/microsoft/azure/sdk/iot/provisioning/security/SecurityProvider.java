package com.microsoft.azure.sdk.iot.provisioning.security;

import javax.net.ssl.SSLContext;

public abstract class SecurityProvider
{
  public static final String DEFAULT_CERT_INSTANCE = "X.509";
  public static final String DEFAULT_TLS_PROTOCOL = "TLSv1.2";
  public static final String DEFAULT_TRUSTED_CERT = "-----BEGIN CERTIFICATE-----\r\\r\n-----END CERTIFICATE-----\r\n-----BEGIN CERTIFICATE-----\r\\r\n-----END CERTIFICATE-----\r\n-----BEGIN CERTIFICATE-----\r\\r\nR9I4LtD+gdwyah617jzV/OeBHRnDJELqYzmp\r\n-----END CERTIFICATE-----\r\n";
  public static final String TRUSTED_CERT_ALIAS = "TRUSTED_CERT_";
  
  /* Error */
  public java.security.KeyStore getKeyStoreWithTrustedCerts()
  {
    // Byte code:
    //   0: invokestatic 30	java/security/KeyStore:getDefaultType	()Ljava/lang/String;
    //   3: invokestatic 34	java/security/KeyStore:getInstance	(Ljava/lang/String;)Ljava/security/KeyStore;
    //   6: astore_1
    //   7: aload_1
    //   8: aconst_null
    //   9: invokevirtual 38	java/security/KeyStore:load	(Ljava/security/KeyStore$LoadStoreParameter;)V
    //   12: ldc 8
    //   14: invokestatic 43	java/security/cert/CertificateFactory:getInstance	(Ljava/lang/String;)Ljava/security/cert/CertificateFactory;
    //   17: astore_2
    //   18: new 45	java/io/ByteArrayInputStream
    //   21: dup
    //   22: ldc 14
    //   24: invokevirtual 51	java/lang/String:getBytes	()[B
    //   27: invokespecial 54	java/io/ByteArrayInputStream:<init>	([B)V
    //   30: astore_3
    //   31: aload_2
    //   32: aload_3
    //   33: invokevirtual 58	java/security/cert/CertificateFactory:generateCertificates	(Ljava/io/InputStream;)Ljava/util/Collection;
    //   36: astore_2
    //   37: aload_3
    //   38: invokevirtual 63	java/io/InputStream:close	()V
    //   41: aload_2
    //   42: invokeinterface 69 1 0
    //   47: astore_2
    //   48: aload_2
    //   49: invokeinterface 75 1 0
    //   54: ifeq +41 -> 95
    //   57: aload_2
    //   58: invokeinterface 79 1 0
    //   63: checkcast 81	java/security/cert/Certificate
    //   66: astore 4
    //   68: ldc 17
    //   70: invokestatic 87	e/a/a/a/a:a	(Ljava/lang/String;)Ljava/lang/StringBuilder;
    //   73: astore_3
    //   74: aload_3
    //   75: invokestatic 93	java/util/UUID:randomUUID	()Ljava/util/UUID;
    //   78: invokevirtual 99	java/lang/StringBuilder:append	(Ljava/lang/Object;)Ljava/lang/StringBuilder;
    //   81: pop
    //   82: aload_1
    //   83: aload_3
    //   84: invokevirtual 102	java/lang/StringBuilder:toString	()Ljava/lang/String;
    //   87: aload 4
    //   89: invokevirtual 106	java/security/KeyStore:setCertificateEntry	(Ljava/lang/String;Ljava/security/cert/Certificate;)V
    //   92: goto -44 -> 48
    //   95: aload_1
    //   96: areturn
    //   97: astore_1
    //   98: aload_1
    //   99: athrow
    //   100: astore_2
    //   101: aload_3
    //   102: invokevirtual 63	java/io/InputStream:close	()V
    //   105: goto +9 -> 114
    //   108: astore_3
    //   109: aload_1
    //   110: aload_3
    //   111: invokevirtual 112	java/lang/Throwable:addSuppressed	(Ljava/lang/Throwable;)V
    //   114: aload_2
    //   115: athrow
    // Local variable table:
    //   start	length	slot	name	signature
    //   0	116	0	this	SecurityProvider
    //   6	90	1	localKeyStore	java.security.KeyStore
    //   97	13	1	localObject1	Object
    //   17	41	2	localObject2	Object
    //   100	15	2	localObject3	Object
    //   30	72	3	localObject4	Object
    //   108	3	3	localThrowable	Throwable
    //   66	22	4	localCertificate	java.security.cert.Certificate
    // Exception table:
    //   from	to	target	type
    //   31	37	97	finally
    //   98	100	100	finally
    //   101	105	108	finally
  }
  
  public abstract String getRegistrationId();
  
  public abstract SSLContext getSSLContext();
}

/* Location:
 * Qualified Name:     com.microsoft.azure.sdk.iot.provisioning.security.SecurityProvider
 * Java Class Version: 6 (50.0)
 * JD-Core Version:    0.7.1
 */