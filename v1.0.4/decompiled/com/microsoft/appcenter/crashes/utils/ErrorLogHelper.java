...
public class ErrorLogHelper
{
  ...
  {
    return FileManager.lastModifiedFile(getErrorStorageDirectory(), new ErrorLogHelper.4());
  }
  ...
  public static File getNewMinidumpDirectory()
  {
    try
    {
      ...
      localFile1 = new File(localFile2, "new");
      return localFile1;
    }
    finally
    {
  ...
  public static File getNewMinidumpSubfolderWithContextData(Context paramContext)
  {
    try
    {
      ...
      try
      {
        Device localDevice = DeviceInfoHelper.getDeviceInfo(paramContext);
        localDevice.setWrapperSdkName("appcenter.ndk");
        paramContext = new org/json/JSONStringer;
        paramContext.<init>();
        paramContext.object();
        localDevice.write(paramContext);
        paramContext.endObject();
        FileManager.write(localFile2, paramContext.toString());
      }
  ...
  {
    paramFile = paramFile.listFiles(new ErrorLogHelper.2());
  ...
  {
    File[] arrayOfFile = getErrorStorageDirectory().listFiles(new ErrorLogHelper.1());
    if (arrayOfFile == null) {
      arrayOfFile = new File[0];
    }
    return arrayOfFile;
  ...
  public static File getStoredFile(UUID paramUUID, String paramString)
  {
    paramUUID = getErrorStorageDirectory().listFiles(new ErrorLogHelper.5(paramUUID, paramString));
  ...
  {
    File[] arrayOfFile = getNewMinidumpDirectory().listFiles(new ErrorLogHelper.3());
  ...
  {
    paramUUID = getStoredErrorLogFile(paramUUID);
    if (paramUUID != null)
    {
      StringBuilder localStringBuilder = a.a("Deleting error log file ");
      localStringBuilder.append(paramUUID.getName());
      AppCenterLog.info("AppCenterCrashes", localStringBuilder.toString());
      FileManager.delete(paramUUID);
    }
  }
  
  public static void removeStoredThrowableFile(UUID paramUUID)
  {
    paramUUID = getStoredThrowableFile(paramUUID);
    if (paramUUID != null)
    {
      StringBuilder localStringBuilder = a.a("Deleting throwable file ");
      localStringBuilder.append(paramUUID.getName());
      AppCenterLog.info("AppCenterCrashes", localStringBuilder.toString());
      FileManager.delete(paramUUID);
    }
...
/* Location:
 * Qualified Name:     base.com.microsoft.appcenter.crashes.utils.ErrorLogHelper
