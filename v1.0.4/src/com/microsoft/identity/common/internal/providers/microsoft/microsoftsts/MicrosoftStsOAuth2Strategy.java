  ...
  extends OAuth2Strategy<MicrosoftStsAccessToken, MicrosoftStsAccount, MicrosoftStsAuthorizationRequest, MicrosoftStsAuthorizationRequest.Builder, AuthorizationStrategy, MicrosoftStsOAuth2Configuration, OAuth2StrategyParameters, MicrosoftStsAuthorizationResponse, MicrosoftStsRefreshToken, MicrosoftStsTokenRequest, MicrosoftStsTokenResponse, TokenResult, AuthorizationResult>
{
  ...
  private String getIssuerCacheIdentifierFromAuthority(URL paramURL)
  {
    ...
    if (localObject != null)
    {
      ...
      localObject = a.a(new StringBuilder(), TAG, ":getIssuerCacheIdentifierFromAuthority");
  ...
  {
    String str = ObjectMapper.serializeObjectToFormUrlEncoded(paramMicrosoftStsTokenRequest);
    paramMicrosoftStsTokenRequest = new TreeMap();
    paramMicrosoftStsTokenRequest.put("client-request-id", DiagnosticContext.getRequestContext().get("correlation_id"));
    paramMicrosoftStsTokenRequest.putAll(Device.getPlatformIdParameters());
    ...
    try
    {
      localObject1 = new com/microsoft/identity/common/internal/ui/webview/challengehandlers/PKeyAuthChallengeFactory;
      ((PKeyAuthChallengeFactory)localObject1).<init>();
      localObject2 = StringExtensions.getUrl(mTokenEndpoint);
      paramMicrosoftStsTokenRequest.putAll(PKeyAuthChallengeHandler.getChallengeHeader(((PKeyAuthChallengeFactory)localObject1).getPKeyAuthChallenge(paramHttpResponse, ((URL)localObject2).toString())));
      paramMicrosoftStsTokenRequest.putAll(EstsTelemetry.getInstance().getTelemetryHeaders());
      paramHttpResponse = HttpRequest.sendPost((URL)localObject2, paramMicrosoftStsTokenRequest, str.getBytes("UTF-8"), "application/x-www-form-urlencoded");
  ...
  private void validateAuthScheme(MicrosoftStsTokenRequest paramMicrosoftStsTokenRequest, MicrosoftStsTokenResponse paramMicrosoftStsTokenResponse)
  {
    ...
    String str = paramMicrosoftStsTokenResponse.getTokenType();
    if ((paramMicrosoftStsTokenRequest != null) && (!paramMicrosoftStsTokenRequest.equalsIgnoreCase(str)))
    {
      paramMicrosoftStsTokenResponse = new StringBuilder();
      paramMicrosoftStsTokenResponse.append("Expected: [");
      paramMicrosoftStsTokenResponse.append(paramMicrosoftStsTokenRequest);
      paramMicrosoftStsTokenResponse.append("]\nActual: [");
      paramMicrosoftStsTokenResponse.append(str);
      paramMicrosoftStsTokenResponse.append("]");
      throw new ClientException("auth_scheme_mismatch", paramMicrosoftStsTokenResponse.toString());
    }
  }
  ...
  public MicrosoftStsTokenRequest createTokenRequest(MicrosoftStsAuthorizationRequest paramMicrosoftStsAuthorizationRequest, MicrosoftStsAuthorizationResponse paramMicrosoftStsAuthorizationResponse, AbstractAuthenticationScheme paramAbstractAuthenticationScheme)
  {
    ...
    if ("PoP".equals(paramAbstractAuthenticationScheme.getName())) {
      if (mStrategyParameters.getContext() != null)
      {
        ...
        if (!paramAbstractAuthenticationScheme.asymmetricKeyExists())
        {
          paramMicrosoftStsAuthorizationRequest = paramAbstractAuthenticationScheme.generateAsymmetricKey(mStrategyParameters.getContext());
          String str = TAG;
          paramMicrosoftStsAuthorizationResponse = new StringBuilder();
          paramMicrosoftStsAuthorizationResponse.append("Generated new PoP asymmetric key with thumbprint: ");
          paramMicrosoftStsAuthorizationResponse.append(paramMicrosoftStsAuthorizationRequest);
          Logger.verbosePII(str, paramMicrosoftStsAuthorizationResponse.toString());
        }
        localMicrosoftStsTokenRequest.setRequestConfirmation(paramAbstractAuthenticationScheme.getRequestConfirmation());
      }
      else
      {
  ...
  public String getIssuerCacheIdentifierFromTokenEndpoint()
  {
    ...
    try
    {
      ...
      localURL1.<init>(mTokenEndpoint);
    }
    catch (MalformedURLException localMalformedURLException)
    {
      StringBuilder localStringBuilder = new StringBuilder();
      localStringBuilder.append(TAG);
      localStringBuilder.append(":getIssuerCacheIdentifierFromTokenEndpoint");
      Logger.error(localStringBuilder.toString(), "Getting issuer cache identifier from token endpoint failed due to malformed URL (mTokenEndpoint)...", localMalformedURLException);
      localURL2 = null;
    }
    if (localURL2 != null) {
      str = getIssuerCacheIdentifierFromAuthority(localURL2);
    }
    return str;
  }
...
/* Location:
 * Qualified Name:     base.com.microsoft.identity.common.internal.providers.microsoft.microsoftsts.MicrosoftStsOAuth2Strategy
