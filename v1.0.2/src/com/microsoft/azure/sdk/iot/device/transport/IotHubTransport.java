  ...
  implements IotHubListener
{
  ...
  {
    Object localObject1 = paramIotHubTransportMessage.getMessageCallback();
    Object localObject2 = paramIotHubTransportMessage.getMessageCallbackContext();
    if (localObject1 != null)
    {
      log.e("Executing callback for received message ({})", paramIotHubTransportMessage);
      localObject1 = ((MessageCallback)localObject1).execute(paramIotHubTransportMessage, localObject2);
      try
      {
        log.e("Sending acknowledgement for received cloud to device message ({})", paramIotHubTransportMessage);
        iotHubTransportConnection.sendMessageResult(paramIotHubTransportMessage, (IotHubMessageResult)localObject1);
      }
      catch (TransportException localTransportException)
      {
        log.b("Sending acknowledgement for received cloud to device message failed, adding it back to the queue ({})", paramIotHubTransportMessage, localTransportException);
  ...
  private void checkForExpiredMessages()
  {
    ...
    synchronized (inProgressMessagesLock)
    {
      ...
      localObject2 = inProgressPackets.keySet().iterator();
  ...
  {
    log.b("Handling an exception from sending message: Attempt number {}", Integer.valueOf(paramIotHubTransportPacket.getCurrentRetryAttempt()), paramTransportException);
    ...
    if (!hasOperationTimedOut(paramIotHubTransportPacket.getStartTimeMillis()))
    {
      if (paramTransportException.isRetryable())
      {
        ...
        {
          taskScheduler.schedule(new IotHubTransport.MessageRetryRunnable(this, waitingPacketsQueue, paramIotHubTransportPacket), ((RetryDecision)localObject).getDuration(), TimeUnit.MILLISECONDS);
          return;
        }
        log.b("Retry policy dictated that the message should be abandoned, so it has been abandoned ({})", paramIotHubTransportPacket.getMessage(), paramTransportException);
      }
      else
      {
        log.b("Encountering an non-retryable exception while sending a message, so it has been abandoned ({})", paramIotHubTransportPacket.getMessage(), paramTransportException);
      }
    }
    else {
      log.b("The device operation timeout has been exceeded for the message, so it has been abandoned ({})", paramIotHubTransportPacket.getMessage(), paramTransportException);
    }
  ...
  private void reconnect(TransportException paramTransportException)
  {
    ...
    try
    {
      ...
      else if (hasOperationTimedOut(reconnectionAttemptStartTimeMillis))
      {
        ...
        paramTransportException = IotHubConnectionStatusChangeReason.RETRY_EXPIRED;
        localObject = new com/microsoft/azure/sdk/iot/device/exceptions/DeviceOperationTimeoutException;
        ((DeviceOperationTimeoutException)localObject).<init>("Device operation for reconnection timed out");
        close(paramTransportException, (Throwable)localObject);
      }
  ...
  private void sendPacket(IotHubTransportPacket paramIotHubTransportPacket)
  {
    ...
    try
    {
      ...
      log.e("Sent message ({}) to protocol level, returned status code was {}", localMessage, localIotHubStatusCode);
    ...
    {
      log.b("Encountered exception while sending message with correlation id {}", localMessage.getCorrelationId(), localTransportException);
      if (i != 0) {
        synchronized (inProgressMessagesLock)
        {
  ...
  private void updateStatus(IotHubConnectionStatus paramIotHubConnectionStatus, IotHubConnectionStatusChangeReason paramIotHubConnectionStatusChangeReason, Throwable paramThrowable)
  {
    if (connectionStatus != paramIotHubConnectionStatus)
    {
      if (paramThrowable == null) {
        log.a("Updating transport status to new status {} with reason {}", paramIotHubConnectionStatus, paramIotHubConnectionStatusChangeReason);
      } else {
        log.c("Updating transport status to new status {} with reason {}", new Object[] { paramIotHubConnectionStatus, paramIotHubConnectionStatusChangeReason, paramThrowable });
      }
  ...
  public void invokeCallbacks()
  {
    for (IotHubTransportPacket localIotHubTransportPacket = (IotHubTransportPacket)callbackPacketsQueue.poll(); localIotHubTransportPacket != null; localIotHubTransportPacket = (IotHubTransportPacket)callbackPacketsQueue.poll())
    {
      ...
      log.a("Invoking the callback function for sent message, IoT Hub responded to message ({}) with status {}", localIotHubTransportPacket.getMessage(), localIotHubStatusCode);
  ...
  public void onConnectionLost(Throwable paramThrowable, String paramString)
  {
    synchronized (reconnectionLock)
    {
      if (!paramString.equals(iotHubTransportConnection.getConnectionId()))
      {
        log.e("OnConnectionLost was fired, but for an outdated connection. Ignoring...");
        return;
      }
      if (connectionStatus != IotHubConnectionStatus.CONNECTED)
      {
        log.e("OnConnectionLost was fired, but connection is already disocnnected. Ignoring...");
        return;
      }
      if ((paramThrowable instanceof TransportException))
      {
  ...
  public void onMessageReceived(IotHubTransportMessage paramIotHubTransportMessage, Throwable paramThrowable)
  {
    ...
    {
      log.d("Exception encountered while receiving a message from service {}", paramIotHubTransportMessage, paramThrowable);
    }
...
/* Location:
 * Qualified Name:     base.com.microsoft.azure.sdk.iot.device.transport.IotHubTransport
