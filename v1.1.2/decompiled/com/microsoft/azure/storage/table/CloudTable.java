...
public final class CloudTable
{
  ...
  private StorageRequest<CloudTableClient, CloudTable, TablePermissions> downloadPermissionsImpl(final TableRequestOptions paramTableRequestOptions)
  {
    new StorageRequest(paramTableRequestOptions, getStorageUri())
    {
      public HttpURLConnection buildRequest(CloudTableClient paramAnonymousCloudTableClient, CloudTable paramAnonymousCloudTable, OperationContext paramAnonymousOperationContext)
      {
        return TableRequest.getAcl(paramAnonymousCloudTable.getStorageUri().getUri(getCurrentLocation()), paramTableRequestOptions, paramAnonymousOperationContext);
      }
      ...
      public TablePermissions postProcessResponse(HttpURLConnection paramAnonymousHttpURLConnection, CloudTable paramAnonymousCloudTable, CloudTableClient paramAnonymousCloudTableClient, OperationContext paramAnonymousOperationContext, TablePermissions paramAnonymousTablePermissions)
      {
        paramAnonymousCloudTableClient = SharedAccessPolicyHandler.getAccessIdentifiers(getConnection().getInputStream(), SharedAccessTablePolicy.class);
        paramAnonymousCloudTable = paramAnonymousCloudTableClient.keySet().iterator();
        while (paramAnonymousCloudTable.hasNext())
        {
          paramAnonymousHttpURLConnection = (String)paramAnonymousCloudTable.next();
          paramAnonymousTablePermissions.getSharedAccessPolicies().put(paramAnonymousHttpURLConnection, paramAnonymousCloudTableClient.get(paramAnonymousHttpURLConnection));
        }
        return paramAnonymousTablePermissions;
      }
      
      public TablePermissions preProcessResponse(CloudTable paramAnonymousCloudTable, CloudTableClient paramAnonymousCloudTableClient, OperationContext paramAnonymousOperationContext)
      {
  ...
  private boolean exists(boolean paramBoolean, TableRequestOptions paramTableRequestOptions, OperationContext paramOperationContext)
  {
    ...
    paramOperationContext = (QueryTableOperation)TableOperation.retrieve(name, null, DynamicTableEntity.class);
    paramOperationContext.setPrimaryOnlyRetrieve(paramBoolean);
    paramTableRequestOptions = paramOperationContext.execute(tableServiceClient, "Tables", paramTableRequestOptions, localOperationContext);
  ...
  private StorageRequest<CloudTableClient, CloudTable, Void> uploadPermissionsImpl(final TablePermissions paramTablePermissions, final TableRequestOptions paramTableRequestOptions)
  {
    ...
    try
    {
      ...
      paramTablePermissions = new StorageRequest(paramTableRequestOptions, getStorageUri())
      {
        public HttpURLConnection buildRequest(CloudTableClient paramAnonymousCloudTableClient, CloudTable paramAnonymousCloudTable, OperationContext paramAnonymousOperationContext)
        {
          setSendStream(new ByteArrayInputStream(paramTablePermissions));
          setLength(Long.valueOf(paramTablePermissions.length));
          return TableRequest.setAcl(paramAnonymousCloudTable.getStorageUri().getUri(getCurrentLocation()), paramTableRequestOptions, paramAnonymousOperationContext);
        }
        ...
        public Void preProcessResponse(CloudTable paramAnonymousCloudTable, CloudTableClient paramAnonymousCloudTableClient, OperationContext paramAnonymousOperationContext)
        {
        ...
        public void signRequest(HttpURLConnection paramAnonymousHttpURLConnection, CloudTableClient paramAnonymousCloudTableClient, OperationContext paramAnonymousOperationContext)
        {
          StorageRequest.signTableRequest(paramAnonymousHttpURLConnection, paramAnonymousCloudTableClient, paramTablePermissions.length, paramAnonymousOperationContext);
        }
      };
...
/* Location:
 * Qualified Name:     com.microsoft.azure.storage.table.CloudTable
